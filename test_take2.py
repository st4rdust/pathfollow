...
# Initialize the variables (i.e. assign their default value) when run.
...
with tf.Session() as sess:
    ...
    #     Runs optimizer on each value (x, y) which uses gradient descent to
    #     minimize the cost function to find the best weights and biases
    #     essentially, follows gradient at (x, y) repeatedly
    for epoch in range(training_epochs):
        for (x, y) in zip(train_X, train_Y):
            sess.run(optimizer, feed_dict={X: x, Y: y})

        # Display logs per epoch step (display_step variable)
